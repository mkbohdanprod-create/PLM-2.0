...
async function fix() {
  ...
  // We need to set the role to SUPER_ADMIN to bypass the transition rules
  await client.query(`SET LOCAL "request.jwt.claim.role" = 'SUPER_ADMIN';`);

  console.log('\\n1. --- Creating Test Order (MEASUREMENT_SCHEDULING) ---');
  let res = await client.query(`
    SELECT * FROM create_order('E2E-TEST-4', $1, 'FULL_CYCLE', 'Test Flow 4', '380991234560', 'Kyiv', 'St', '1', 'PVC', 10, true, null, null, null, null, null, null);
  `, [valid_branch]);
