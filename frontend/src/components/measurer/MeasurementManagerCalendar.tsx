...
export function MeasurementManagerCalendar({ refreshTrigger, globalRegion = ['Всі'], measurers = [] }: MeasurementManagerCalendarProps) {
  ...
  const fetchData = async () => {
    ...
    if (measurers && measurers.length > 0) {
      const { data: scheds } = await supabase
        ...
        .select('*')
        .in('profile_id', measurers.map(p => p.id))
        .gte('work_date', sDate)
        .lte('work_date', eDate);
      setSchedules(scheds || []);
    }
    ...
    const { data: measurementTasks } = await supabase
      ...
      .select(`
        *, profiles (full_name, color),
        orders!inner ( status, order_type, order_number, external_id, is_hidden, order_addresses ( city, street, building ), order_specifications ( area_sqm ), branches ( regions (name) ) )
      `)
  ...
  const getDayWorkload = (dateStr: string) => {
    ...
    const totalPlannedMins = allTasksForDay.reduce((sum, task) => {
      let taskDuration = defaultDuration;
      if (task.start_time && task.end_time) {
        const [sh, sm] = task.start_time.split(':').map(Number);
        const [eh, em] = task.end_time.split(':').map(Number);
        taskDuration = (eh * 60 + (em || 0)) - (sh * 60 + (sm || 0));
      } else if (task.orders?.measurement_duration_mins) {
        taskDuration = task.orders.measurement_duration_mins;
      }
      return sum + Math.max(0, taskDuration) + (task.estimated_travel_time_mins || defaultTravel);
    }, 0);
  ...
  const getRegionDayWorkload = (regionName: string, dateStr: string) => {
    ...
    let totalCapacityMins = 0;
    const regionMeasurers = measurers.filter(m => (m.branches?.regions?.name || 'Інше') === regionName);
  ...
  const getDayTasks = (measurerId: string | null, dateStr: string, regionName?: string) => {
    return tasks.filter(t => {
      const matchMeasurer = t.measurer_id === measurerId;
  ...
  const filteredMeasurers = measurers.filter(m => filterMeasurerId === 'ALL' || m.id === filterMeasurerId);
  
  const regionsSet = new Set<string>();
  filteredMeasurers.forEach(m => regionsSet.add(m.regions?.name || m.branches?.regions?.name || 'Інше'));
  
  if (filterMeasurerId === 'ALL' || filterMeasurerId === 'unassigned') {
    tasks.forEach(t => {
      if (t.measurer_id === null && filterTaskByStatus(t)) {
        regionsSet.add(t.orders?.branches?.regions?.name || 'Інше');
      }
    });
  }
...
function DraggableCalendarTask({ task, color = 'var(--accent-color)', isSelected, onSelectOrder }: { task: any, color?: string, isSelected?: boolean, onSelectOrder?: (id: string) => void }) {
  const isLocked = !['MEASUREMENT_SCHEDULING', 'MEASUREMENT_PRE_SCHEDULED'].includes(task.orders?.status) && !isPaused(task.orders?.status);
  ...
  const tooltipText = `Замовлення: ${task.orders?.external_id || task.orders?.order_number || 'Без номера'}
Статус: ${STATUS_LABELS[task.orders?.status] || task.orders?.status || 'Невідомо'}
Адреса: ${task.orders?.order_addresses?.[0] ? `${task.orders.order_addresses[0].city || ''}, ${task.orders.order_addresses[0].street || ''}, ${task.orders.order_addresses[0].building || ''}`.replace(/^[,\s]+|[,\s]+$/g, '') : 'Не вказана'}
Тип: ${ORDER_TYPES[task.orders?.order_type] || task.orders?.order_type || 'Не вказано'}
Площа: ${task.orders?.order_specifications?.[0]?.area_sqm || 0} м²
Замірник: ${task.profiles?.full_name || 'Не призначено'}`;
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '12px', color: actualColor, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {task.orders?.external_id || task.orders?.order_number || 'Без номера'}
      </div>
