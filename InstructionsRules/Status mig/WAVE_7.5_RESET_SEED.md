...
## Phase 0: Data Inventory (Before Cleanup)
Here are the current row counts from the database to ensure we know exactly what is being cleared.

**Transactional Tables (To be cleaned):**
- `orders`: 23
- `order_status_history`: 67
- `order_contacts`: 11
- `order_addresses`: 11
- `order_specifications`: 6
- `measurement_tasks`: 2
- `engineering_tasks`: 5
- `webhook_events`: 7
- `audit_logs`: 138 (See open question below)
- `delivery_tasks`: 0
- `order_activities`: 0

**Dictionary Tables (Will NOT be touched during reset):**
- `regions`: 4
- `branches`: 6
- `materials`: 5
- `roles`: 7
...
> [!IMPORTANT] 
> **Open Question:** `audit_logs` contains 138 records. I have commented out the truncation of `audit_logs` in the script below to preserve history as you suggested. Should I leave it commented out, or do you prefer a completely clean slate including audit logs?

---

## Phase 1: Cleanup Script (`reset_test_data.sql`)
This script safely drops all transactional data using `CASCADE` without touching dictionaries.
...
## Phase 2: Seed Reference Data (`seed_reference_data.sql`)
This script injects the reference dictionaries, creates personnel, and builds the 3 baseline orders for testing.
...
INSERT INTO public.roles (code, name, is_system) VALUES ('LOGIST', 'Логіст (Водій)', false) ON CONFLICT DO NOTHING;
...
-- Create helper function to insert auth users safely
DO $$
DECLARE
...
BEGIN
  ...
  -- The trigger uses `id` as primary key.
...
-- 7. BASELINE ORDERS
-- create_order signature: create_order(p_external_id, p_branch_id, p_order_type, p_full_name, p_phone, p_city, p_street, p_building, p_material, p_area, p_force)
-- p_material takes text!
DO $$
DECLARE
  v_kyiv uuid; v_lviv uuid;
...
BEGIN
  ...
  SELECT public.create_order('ORD-001', v_kyiv, 'FULL_CYCLE', 'Клієнт 1', '380501112233', 'Київ', 'Хрещатик', '1', 'Акрил Pure', 3.5) INTO v_id1;

  -- Order 2: Engineering (ENGINEERING_QUEUE)
  SELECT public.create_order('ORD-002', v_lviv, 'FULL_CYCLE', 'Клієнт 2', '380671112233', 'Львів', 'Площа Ринок', '1', 'Граніт Black Galaxy', 5.0) INTO v_id2;
  ...
  SELECT public.create_order('ORD-003', v_kyiv, 'FULL_CYCLE', 'Клієнт 3', '380631112233', 'Київ', 'Перемоги', '50', 'Кварц White', 2.8) INTO v_id3;
