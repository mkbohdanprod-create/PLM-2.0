...
| **Час на дорогу** | Інтервал для планування добирання диспетчером. | Автоматично (OSRM) / Ручне введення | Час (Хвилини) |
| **Час на замір** | Динамічно розраховується: Якщо квадратура не вказана = 2 год. Якщо вказана: до 6 кв.м = 1-1.5 год, 6-10 кв.м = 2 год, від 10 кв.м = 4 год, >15 кв.м = індивідуально. Може бути перевизначено диспетчером. | Автоматика / Ручно | Час (Хвилини) |
...
*   **Права частина (Основна робоча область):**
    *   **Верхній блок (Календар / Timeline):** Відображає завантаженість працівників (замірників). Це не просто табличка, а інтелектуальна система, яка синхронізована з налаштуваннями керівника ("Графік роботи працівників"). Календар показує, у які дні та години конкретний виконавець доступний, щоб оператор не міг призначити виїзд на вихідний чи лікарняний. У правому куті є перемикачі масштабу: **День, Тиждень, Місяць**.
        *   *(✅ Реалізовано в Етапі 3: Підтримка drag & drop, черга "Без замірника" та візуальний розрахунок навантаження у вигляді "батарейки". Алгоритм навантаження на день враховує:*
            *1. Точний час у розкладі (end_time - start_time) якщо завдання заплановано.*
            *2. Індивідуальний "Необхідний час на замір" (measurement_duration_mins), якщо конкретний час ще не визначено.*
            *3. Час в дорозі від/до об'єкта.*
            *Сума цих показників ділиться на доступну денну ємність спеціаліста (стандартний робочий день × коефіцієнт ефективності), що відображає максимально точний % навантаження).*
...
- **Фільтрація по макро-етапам:** Усі фільтри у UI тепер опираються виключно на 9 Макро-етапів (напр. 'MEASUREMENT_SCHEDULING' та 'MEASUREMENT'), а не на технічні підстатуси БД.
- **OSRM та розрахунок часу:** Маршрутизатор (OSRM) обчислює час добирання ТІЛЬКИ для активних задач (статуси SCHEDULED та IN_PROGRESS).
- **Скасування / Відв'язка:** При скасуванні заміру (повернення в MEASUREMENT_SCHEDULING) замовлення отримує фору в 4 години (planned_call_date = now() + 4 hours), щоб диспетчер мав час на реакцію. При простому знятті з календаря без переведення статусу (unassign), дата продзвону встановлюється на поточний момент (
ow()) із обов'язковим коментарем 'Потрібен повторний контакт'.
- **Комунікації:** Автоматично успадковують macro_stage замовлення (або дефолтно MEASUREMENT_SCHEDULING для нових) та обов'язково зв'язуються з public.profiles для ідентифікації автора/виконавця.
