-- 1. Add is_reclamation_frozen to orders
ALTER TABLE public.orders ADD COLUMN is_reclamation_frozen boolean DEFAULT false;

-- 2. Add INSTALLATION_RECLAMATION transitions
...
-- 3. Modify change_order_status to block COMPLETED
...
-- 4. Create create_reclamation RPC
CREATE OR REPLACE FUNCTION public.create_reclamation(
  p_parent_order_id uuid,
  p_reason text,
  p_reclamation_type text,
  p_return_to_stage text,
  p_source text,
...
BEGIN
  ...
  -- 2. Prevent recursion
  SELECT * INTO v_parent_rec FROM public.orders WHERE id = p_parent_order_id;
  IF NOT FOUND THEN
     RAISE EXCEPTION 'Parent order not found';
  END IF;
  
  IF v_parent_rec.parent_order_id IS NOT NULL THEN
     RAISE EXCEPTION 'Cannot create reclamation for a child order';
  END IF;

  -- 3. Number generation
  SELECT COUNT(*) INTO v_child_count FROM public.orders WHERE parent_order_id = p_parent_order_id;
  v_new_order_number := v_parent_rec.order_number || '-R' || (v_child_count + 1)::text;

  -- 4. Create child order
  INSERT INTO public.orders (
    order_number, branch_id, status, order_type, parent_order_id, external_id, document_date
  ) VALUES (
    v_new_order_number, v_parent_rec.branch_id, p_return_to_stage, v_parent_rec.order_type, p_parent_order_id, v_parent_rec.external_id, current_date
  ) RETURNING id INTO v_new_order_id;

  -- Copy contacts
  INSERT INTO public.order_contacts (order_id, full_name, phone, email, role)
  SELECT v_new_order_id, full_name, phone, email, role
  FROM public.order_contacts WHERE order_id = p_parent_order_id;

  -- Copy addresses
  INSERT INTO public.order_addresses (order_id, city, street, building, lat, lng, is_manually_pinned)
  SELECT v_new_order_id, city, street, building, lat, lng, is_manually_pinned
  FROM public.order_addresses WHERE order_id = p_parent_order_id;

  -- Copy specifications
  INSERT INTO public.order_specifications (order_id, material_type, area_sqm, total_amount, material_id, decor_id)
  SELECT v_new_order_id, material_type, area_sqm, total_amount, material_id, decor_id
  FROM public.order_specifications WHERE order_id = p_parent_order_id;

  -- 5. Freeze parent order
  PERFORM set_config('app.source', p_source, true);
  
  IF p_reclamation_type = 'INSTALLATION' THEN
     -- Call change_order_status to properly log the status change
     PERFORM public.change_order_status(p_parent_order_id, 'INSTALLATION_RECLAMATION', p_reason);
     UPDATE public.orders SET is_reclamation_frozen = true WHERE id = p_parent_order_id;
  ELSE
     -- Just freeze, keep status
     UPDATE public.orders SET is_reclamation_frozen = true WHERE id = p_parent_order_id;
  END IF;

  -- 6. Log idempotency
  INSERT INTO public.webhook_events (idempotency_key, source, payload, processed_at)
  VALUES (p_idempotency_key, p_source, jsonb_build_object('parent_order_id', p_parent_order_id, 'new_order_id', v_new_order_id, 'reason', p_reason), now());

  RETURN v_new_order_id;
END;
$function$;

-- 5. Create trg_reclamation_unfreeze_parent
CREATE OR REPLACE FUNCTION public.reclamation_unfreeze_parent()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
...
BEGIN
  IF NEW.status = 'COMPLETED' AND OLD.status != 'COMPLETED' AND NEW.parent_order_id IS NOT NULL THEN
     
     -- Only unfreeze if this was the last active child reclamation
     ...
     IF v_active_children = 0 THEN
         UPDATE public.orders SET is_reclamation_frozen = false WHERE id = NEW.parent_order_id;
         
         SELECT status INTO v_parent_status FROM public.orders WHERE id = NEW.parent_order_id;
         IF v_parent_status = 'INSTALLATION_RECLAMATION' THEN
            PERFORM public.change_order_status(NEW.parent_order_id, 'INSTALLATION_SCHEDULING');
         END IF;
     END IF;
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS trg_reclamation_unfreeze_parent_update ON public.orders;
CREATE TRIGGER trg_reclamation_unfreeze_parent_update
AFTER UPDATE OF status ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.reclamation_unfreeze_parent();
