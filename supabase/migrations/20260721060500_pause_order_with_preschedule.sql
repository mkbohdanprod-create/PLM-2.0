-- Migration to handle pausing an order with optional pre-scheduling to "Без замірника"

CREATE OR REPLACE FUNCTION public.pause_order_with_preschedule(
  p_order_id uuid,
  p_reason text,
  p_reason_id uuid,
  p_planned_call_date timestamp with time zone,
  p_call_comment text,
  p_pre_scheduled_date date
) RETURNS boolean AS $$
BEGIN
  ...
  IF p_pre_scheduled_date IS NOT NULL THEN
    ...
    INSERT INTO public.measurement_tasks (
      order_id, measurer_id, scheduled_date, outcome
    ) VALUES (
      p_order_id, NULL, p_pre_scheduled_date, 'SCHEDULED'
    );
  END IF;
