-- 20260702151000_fix_rpc_overload.sql

BEGIN;

-- Drop the old 5-arg version (returns void)
DROP FUNCTION IF EXISTS public.assign_measurement(uuid, uuid, date, time, time);

-- Drop the old 6-arg version (returns uuid)
DROP FUNCTION IF EXISTS public.assign_measurement(uuid, uuid, date, time, time, integer);

-- Recreate a single, definitive 6-arg version that returns void,
-- combining the logic from stage3_fixes with the travel time parameter from stage3_rpc
