  ...
  try {
    await client.query(`
      ...
      BEGIN
        ...
        INSERT INTO public.orders (order_number, external_id, branch_id, status, order_type, delivery_method, is_incomplete, document_date, base_readiness_date, payment_date, calc_readiness_date, planned_call_date, call_comment)
        VALUES ('O-' || upper(substr(md5(random()::text), 1, 6)), p_external_id, p_branch_id, v_initial_status, p_order_type, COALESCE(p_delivery_method, 'DELIVERY'), v_is_incomplete, p_document_date, p_base_readiness_date, p_payment_date, p_calc_readiness_date, v_planned_call_date, CASE WHEN v_planned_call_date IS NOT NULL THEN 'Нове замовлення' ELSE NULL END)
      ...
      $function$;
    `);
    console.log('Fixed create_order RPC in DB');
  } catch(e) {
