...
async function run() {
  ...
  try {
    ...
    `);
    console.log('Successfully altered delivery_tasks columns to nullable');
    
    // Check if constraint exists, if not, add it
    const cRes = await client.query(`
      SELECT conname FROM pg_constraint WHERE conrelid = 'delivery_tasks'::regclass AND conname = 'delivery_tasks_outcome_check'
    `);
    if (cRes.rows.length === 0) {
        ...
        `);
        console.log('Added CHECK constraint on outcome');
    }
